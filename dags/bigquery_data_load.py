...
with DAG(
    ...
    load_data = GoogleCloudStorageToBigQueryOperator(
        task_id="load_data",
        bucket='rm-logistics-landing-bucket-dev',
        source_objects=["*"],
        source_format="CSV",
        skip_leading_rows=1,
        field_delimiter=",",
        destination_project_dataset_table="rm-airflow-tutorial-demo.vehicle_analytics.history",
    ...
    query = """
        ...
        FROM (
            ...
            FROM `rm-airflow-tutorial-demo.vehicle_analytics.history`) as latest
        WHERE rank = 1;
        """
    ...
    create_table = BigQueryOperator(
        ...
        sql=query,
        destination_dataset_table="rm-airflow-tutorial-demo.vehicle_analytics.latest",
